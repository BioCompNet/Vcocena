...
cutoff_prep=function(cutoff, corrdf_r, print.all.plots, global_set, x){
  ...
  rsquaredfun = function(graph_df, cutoff){
    ...
    num_networks = graph_components$csize[graph_components$csize >= global_set$min_nodes_number_for_network] %>% 
      length()
    ...
    if(length(probability)==0){
      ...
      if(print.all.plots) {
        ...
        degree_distribution_wd<-paste0("dir_DegreeDistribution_", layer_settings[[paste0("set", x)]]$top_var)
        ...
        ggsave(path = paste0(working_directory,degree_distribution_wd),
               filename = paste0("Degree_distribution_plot_", cutoff,"_set_", x, ".pdf"),
