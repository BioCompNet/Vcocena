correlation_actions <- function(dd2, layer_set){ 
  
  
  print(paste0("...calculating pairwise correlations..."))
  ...
  range_cutoff<-seq(from = layer_set$min_corr , to = max(correlation_df$rval) , 
                    length.out = layer_set$range_cutoff_length)
  range_cutoff<-round(range_cutoff, 3)
  if(length(range_cutoff)> layer_set$range_cutoff_length) {
